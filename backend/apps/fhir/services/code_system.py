...
class CodeSystemLookupService:
    @staticmethod
    def get_by_id(resource_id: str, tenant_id: str) -> Optional[Any]:
        return FHIRCodeSystemRegistration.all_objects.select_related("version").filter(
    ...
    def process_domain_command(domain_command: dict, context: dict) -> Any:
        raise NotImplementedError("CodeSystem modification via FHIR is not supported.")
