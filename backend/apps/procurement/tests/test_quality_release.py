...
class QualityReleaseTest(TestCase):
    def setUp(self):
        ...
        self.po = PurchaseOrder.objects.create(tenant=self.tenant, po_number="PO-QR", supplier=self.supplier, ordering_branch=self.branch, order_date=timezone.localdate(), expected_delivery_date=timezone.localdate(), status=PurchaseOrder.Status.SENT)
        ...
        self.batch = ReceivedBatch.objects.create(
            ...
            expiry_date=timezone.localdate() + datetime.timedelta(days=365),
