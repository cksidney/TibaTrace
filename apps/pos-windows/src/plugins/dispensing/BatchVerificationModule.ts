import { BatchVerificationResponse, PosDispensingClient } from './types.js';

export class BatchVerificationModule {
  private client: PosDispensingClient;

  constructor(client: PosDispensingClient) {
    this.client = client;
  }

  async verifyScannedBarcode(skuId: string, barcodeRaw: string, quantityScanned: string = '1'): Promise<BatchVerificationResponse> {
    // Parse GS1 DataMatrix or standard barcode string (format: BATCH:EXP:SKU or raw batch number)
    let batchNumber = barcodeRaw.trim();
    let expiryDate: string | null = null;

    if (barcodeRaw.includes('|')) {
      const parts = barcodeRaw.split('|');
      batchNumber = parts[0].trim();
      expiryDate = parts[1] ? parts[1].trim() : null;
    }

    return this.client.verifyBatch({
      sku_id: skuId,
      batch_number: batchNumber,
      expiry_date: expiryDate,
      quantity_scanned: quantityScanned,
    });
  }
}
